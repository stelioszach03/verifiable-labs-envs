...
def test_generate_instance_runs():
    ...
    assert inst.seed == 0
    assert inst.function_signature
    assert inst.docstring
...
def test_generate_instance_seed_determinism():
    try:
        ...
        pytest.skip("generate_problem still NotImplemented")
    assert a.function_signature == b.function_signature
    assert a.gold_solution == b.gold_solution
    assert a.hidden_tests == b.hidden_tests
...
def test_as_inputs_excludes_oracle_fields():
    """Solvers must not see the hidden tests or gold solution."""
    try:
        inst = generate_instance(seed=0)
    except NotImplementedError:
        pytest.skip("generate_problem still NotImplemented")
