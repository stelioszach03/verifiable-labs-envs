"""Multi-turn MRI knee reconstruction — 3-turn rollout with k-space-residual feedback."""
...
class MRIKneeMultiturnEnv(MRIKneeEnv):
    name: str = NAME
...
def load_environment(
    ...
    *,
    ...
    if calibration_quantile is not None:
        ...
        q = _cached_quantile(n_samples=20, alpha=float(_BASE_DEFAULTS["alpha"]))
    else:
        q = _cached_quantile(n_samples=100, alpha=float(_BASE_DEFAULTS["alpha"]))
    return MRIKneeMultiturnEnv(conformal_quantile=q, max_turns=max_turns)


__all__ = ["NAME", "DEFAULT_MAX_TURNS", "MRIKneeMultiturnEnv", "load_environment"]
