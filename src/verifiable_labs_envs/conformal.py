"""Split-conformal prediction utilities used as the uncertainty-calibrated reward term.

Given a calibration set of paired `(x_hat_i, x_true_i, sigma_hat_i)` tuples we pool
per-entry standardized absolute residuals ``r_i = |x_hat_i - x_true_i| / (sigma_hat_i + eps)``
and take the ``(1 - alpha)`` empirical quantile ``q_alpha``. At test time, the
standardized-conformal interval ``[x_hat - q_alpha * sigma_hat, x_hat + q_alpha * sigma_hat]``
has marginal coverage ``>= 1 - alpha`` under exchangeability (Lei et al., 2018).

The reward for a predictor is maximized when its empirical coverage on held-out
instances matches the target ``1 - alpha`` — punishing both overconfident (too-narrow)
and overly-conservative (too-wide) uncertainty estimates.
"""
