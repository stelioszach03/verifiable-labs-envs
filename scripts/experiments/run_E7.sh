#!/usr/bin/env bash
# scripts/experiments/run_E7.sh
#
# E7 — Student size scaling. Two sequential sub-runs:
#   (a) Qwen/Qwen2.5-1.5B-Instruct   (current default)
#   (b) meta-llama/Llama-3.2-3B-Instruct   (memory headroom check)
#
# For the 3B run on 32 GB VRAM we drop NUM_GENERATIONS to 2 and lean
# harder on gradient checkpointing.
#
...
export EXPERIMENT_ID="E7"
export EXPERIMENT_DESCRIPTION="E7 — Student size scaling (Qwen-1.5B vs Llama-3B)"
...
{
  "subruns": [
    {"label": "qwen-1.5b", "model": "${VLABS_BASE_MODEL}",     "num_generations": ${NUM_GENERATIONS}},
    {"label": "llama-3b",  "model": "${VLABS_LARGE_STUDENT}",  "num_generations": 2}
  ],
...
run_subrun() {
    ...
    mkdir -p "${subrun_dir}"
    log_info "subrun: ${label} (model=${model})"
...
if [[ "${DRY_RUN}" == "true" ]]; then
    run_subrun qwen-1.5b "${VLABS_BASE_MODEL}"
    run_subrun llama-3b "${VLABS_LARGE_STUDENT}"
    log_info "[${EXPERIMENT_ID}] dry-run OK (both students)"
    exit 0
fi
