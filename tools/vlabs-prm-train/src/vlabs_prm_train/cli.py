"""``vlabs-prm-train`` Typer CLI — Phase 30.C scaffold.

Subcommands:

- ``train`` — full training run. Refuses to proceed in 30.C; the GPU
  path lights up in 30.F when credits resolve.
- ``dry-run`` — print the resolved config + dependency status without
  running training.
- ``dependencies`` — list which heavy deps are present + which W&B
  credentials are wired up.
- ``version`` — print the package version.
- ``checkpoints`` — list local PRM checkpoints + their manifests.
"""
from __future__ import annotations

import json
from pathlib import Path
from typing import Annotated

import typer

from vlabs_prm_train import __version__

app = typer.Typer(
    name="vlabs-prm-train",
    help="Train the process reward model (Phase 30.C scaffolding; GPU runs gated to 30.F).",
    no_args_is_help=True,
    add_completion=False,
)


def _build_config(
    *,
    dataset: Path,
    base_model: str,
    base_rm_checkpoint: Path | None,
    output_dir: Path,
    eval_set: Path | None,
    calib_set: Path | None,
    lr: float,
    epochs: int,
    batch_size: int,
    grad_accum: int,
    lora_r: int,
    lora_alpha: int,
    wandb_project: str,
    wandb_mode: str,
    seed: int,
    multi_task: bool,
    multi_task_outcome_weight: float,
    max_steps_per_trace: int,
):
    from verifiable_labs_envs.process_reward.trainer import PrmTrainingConfig

    return PrmTrainingConfig(
        base_model=base_model,
        base_rm_checkpoint=str(base_rm_checkpoint) if base_rm_checkpoint else None,
        output_dir=str(output_dir),
        dataset_path=str(dataset),
        eval_dataset_path=str(eval_set) if eval_set else None,
        calib_dataset_path=str(calib_set) if calib_set else None,
        lr=float(lr),
        epochs=int(epochs),
        batch_size=int(batch_size),
        grad_accum=int(grad_accum),
        max_steps_per_trace=int(max_steps_per_trace),
        lora_r=int(lora_r),
        lora_alpha=int(lora_alpha),
        wandb_project=wandb_project,
        wandb_mode=wandb_mode,
        seed=int(seed),
        multi_task=bool(multi_task),
        multi_task_outcome_weight=float(multi_task_outcome_weight),
    )


@app.command()
def version() -> None:
    """Print the CLI version and exit."""
    typer.echo(f"vlabs-prm-train v{__version__}")


@app.command()
def dependencies() -> None:
    """Probe for the GPU-training toolchain."""
    from verifiable_labs_envs.process_reward.trainer import validate_dependencies
    from verifiable_labs_envs.process_reward.wandb_integration import (
        has_wandb_credentials,
        is_wandb_available,
    )

    status = validate_dependencies()
    payload = {
        **status.to_dict(),
        "wandb_installed": is_wandb_available(),
        "wandb_credentials": has_wandb_credentials(),
        "gpu_path_ready": status.is_satisfied,
    }
    typer.echo(json.dumps(payload, indent=2, sort_keys=True))


@app.command("dry-run")
def dry_run(
    dataset: Annotated[
        Path, typer.Option("--dataset", help="Training JSONL path.")
    ] = Path("reports/process_reward/v0.0.1_train.jsonl"),
    base_model: Annotated[
        str, typer.Option("--base-model")
    ] = "Qwen/Qwen2.5-1.5B-Instruct",
    base_rm_checkpoint: Annotated[
        Path | None,
        typer.Option(
            "--base-rm-checkpoint",
            help="D13-B/C shared backbone: path to a Phase 29 RM checkpoint dir.",
        ),
    ] = None,
    output_dir: Annotated[
        Path, typer.Option("--output-dir")
    ] = Path("runs/prm-train/exp_001"),
    eval_set: Annotated[Path | None, typer.Option("--eval-set")] = None,
    calib_set: Annotated[Path | None, typer.Option("--calib-set")] = None,
    lr: Annotated[float, typer.Option("--lr")] = 1e-4,
    epochs: Annotated[int, typer.Option("--epochs", min=1)] = 3,
    batch_size: Annotated[int, typer.Option("--batch-size", min=1)] = 8,
    grad_accum: Annotated[int, typer.Option("--grad-accum", min=1)] = 8,
    lora_r: Annotated[int, typer.Option("--lora-r", min=1)] = 16,
    lora_alpha: Annotated[int, typer.Option("--lora-alpha", min=1)] = 32,
    wandb_project: Annotated[
        str, typer.Option("--wandb-project")
    ] = "vlabs-prm-distillation",
    wandb_mode: Annotated[
        str,
        typer.Option("--wandb-mode", help="online / offline / disabled"),
    ] = "offline",
    seed: Annotated[int, typer.Option("--seed")] = 0,
    multi_task: Annotated[
        bool,
        typer.Option(
            "--multi-task/--no-multi-task",
            help="D4-D multi-task: train per-step + outcome heads jointly.",
        ),
    ] = False,
    multi_task_outcome_weight: Annotated[
        float, typer.Option("--multi-task-outcome-weight", min=0.0, max=1.0)
    ] = 0.3,
    max_steps_per_trace: Annotated[
        int, typer.Option("--max-steps-per-trace", min=1)
    ] = 32,
    write_run_card_to_disk: Annotated[
        bool,
        typer.Option(
            "--write-run-card/--no-write-run-card",
            help="Persist run_card.json under output_dir.",
        ),
    ] = False,
) -> None:
    """Print the resolved training config + dependency status."""
    from verifiable_labs_envs.process_reward.multi_task import (
        MultiTaskConfig,
        is_shared_backbone_ready,
        loss_summary,
    )
    from verifiable_labs_envs.process_reward.trainer import (
        build_training_args,
        validate_dependencies,
        write_run_card,
    )

    config = _build_config(
        dataset=dataset,
        base_model=base_model,
        base_rm_checkpoint=base_rm_checkpoint,
        output_dir=output_dir,
        eval_set=eval_set,
        calib_set=calib_set,
        lr=lr,
        epochs=epochs,
        batch_size=batch_size,
        grad_accum=grad_accum,
        lora_r=lora_r,
        lora_alpha=lora_alpha,
        wandb_project=wandb_project,
        wandb_mode=wandb_mode,
        seed=seed,
        multi_task=multi_task,
        multi_task_outcome_weight=multi_task_outcome_weight,
        max_steps_per_trace=max_steps_per_trace,
    )
    status = validate_dependencies()
    args = build_training_args(config)
    mt_config = MultiTaskConfig(
        per_step_weight=1.0 - multi_task_outcome_weight,
        outcome_weight=multi_task_outcome_weight,
        share_backbone=config.shared_backbone,
        enable=multi_task,
    )

    payload = {
        "config": config.to_dict(),
        "training_args": args,
        "multi_task": loss_summary(mt_config),
        "dependencies": status.to_dict(),
        "shared_backbone_ready": is_shared_backbone_ready(
            config.base_rm_checkpoint
        ),
        "ready_to_train": status.is_satisfied,
    }
    if write_run_card_to_disk:
        target = write_run_card(output_dir, config, status)
        payload["run_card_path"] = str(target)
    typer.echo(json.dumps(payload, indent=2, sort_keys=True))


@app.command()
def train(
    dataset: Annotated[
        Path, typer.Option("--dataset", help="Training JSONL path.")
    ],
    base_model: Annotated[
        str, typer.Option("--base-model")
    ] = "Qwen/Qwen2.5-1.5B-Instruct",
    base_rm_checkpoint: Annotated[
        Path | None, typer.Option("--base-rm-checkpoint")
    ] = None,
    output_dir: Annotated[
        Path, typer.Option("--output-dir")
    ] = Path("runs/prm-train/exp_001"),
    eval_set: Annotated[Path | None, typer.Option("--eval-set")] = None,
    calib_set: Annotated[Path | None, typer.Option("--calib-set")] = None,
    lr: Annotated[float, typer.Option("--lr")] = 1e-4,
    epochs: Annotated[int, typer.Option("--epochs", min=1)] = 3,
    batch_size: Annotated[int, typer.Option("--batch-size", min=1)] = 8,
    grad_accum: Annotated[int, typer.Option("--grad-accum", min=1)] = 8,
    lora_r: Annotated[int, typer.Option("--lora-r", min=1)] = 16,
    lora_alpha: Annotated[int, typer.Option("--lora-alpha", min=1)] = 32,
    wandb_project: Annotated[
        str, typer.Option("--wandb-project")
    ] = "vlabs-prm-distillation",
    wandb_mode: Annotated[str, typer.Option("--wandb-mode")] = "offline",
    seed: Annotated[int, typer.Option("--seed")] = 0,
    multi_task: Annotated[
        bool, typer.Option("--multi-task/--no-multi-task")
    ] = False,
    multi_task_outcome_weight: Annotated[
        float, typer.Option("--multi-task-outcome-weight", min=0.0, max=1.0)
    ] = 0.3,
    max_steps_per_trace: Annotated[
        int, typer.Option("--max-steps-per-trace", min=1)
    ] = 32,
) -> None:
    """Run the full PRM training loop. **Gated to 30.F.**"""
    from verifiable_labs_envs.process_reward.trainer import (
        GpuPathNotImplemented,
        build_prm_trainer,
        validate_dependencies,
    )

    config = _build_config(
        dataset=dataset,
        base_model=base_model,
        base_rm_checkpoint=base_rm_checkpoint,
        output_dir=output_dir,
        eval_set=eval_set,
        calib_set=calib_set,
        lr=lr,
        epochs=epochs,
        batch_size=batch_size,
        grad_accum=grad_accum,
        lora_r=lora_r,
        lora_alpha=lora_alpha,
        wandb_project=wandb_project,
        wandb_mode=wandb_mode,
        seed=seed,
        multi_task=multi_task,
        multi_task_outcome_weight=multi_task_outcome_weight,
        max_steps_per_trace=max_steps_per_trace,
    )
    status = validate_dependencies()
    if not status.is_satisfied:
        typer.echo(
            "ABORT: missing dependencies — install vlabs-prm-train[gpu] "
            f"to pull: {', '.join(status.missing)}",
            err=True,
        )
        raise typer.Exit(code=2)
    try:
        build_prm_trainer(config)
    except GpuPathNotImplemented as exc:
        typer.echo(f"ABORT: {exc}", err=True)
        raise typer.Exit(code=3) from exc


@app.command()
def checkpoints(
    parent: Annotated[
        Path,
        typer.Option(
            "--parent",
            help="Parent dir holding PRM manifest.json files.",
        ),
    ] = Path("runs/prm-train"),
) -> None:
    """List local PRM checkpoints + their manifests."""
    from verifiable_labs_envs.process_reward.checkpoint import (
        list_local_checkpoints,
        manifest_table,
    )

    manifests = list_local_checkpoints(parent)
    typer.echo(json.dumps(manifest_table(manifests), indent=2, sort_keys=True))


__all__ = ["app"]
